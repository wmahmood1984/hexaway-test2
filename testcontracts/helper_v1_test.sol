...
contract Helper is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function goDistribute(
        ...
        paymentToken.transfer(oldOwner, ((amount * 10) / percentageAtBuy));
        selfTradingProfit[oldOwner] += ((amount * 10) / percentageAtBuy);
        emit Incomes(
            block.timestamp,
    ...
    function stakeAndBurn(address _user) public {
        require(msg.sender == maintenanceWallet, "AUTH: not authorized");

        NFT[] memory userNFTs = getNFTs(_user);

        for (uint i = 0; i < userNFTs.length; i++) {
            uint id = userNFTs[i].id;

            require(
                id > 0,
                string(
                    abi.encodePacked(
                        "BURN: id underflow at userNFTs index ",
                        _toString(i),
                        ", id=0"
                    )
                )
        ...
        NFT[] memory burntNFTs = getNFTused();

        for (int i = int(burntNFTs.length) - 1; i >= 0; i--) {
            if (burntNFTs[uint(i)]._owner == _user) {
        removeFirst2(uint(i));
    }
            }
        

        queIncome[] memory NFTQueLocal = getNFTque();

        for (int i = int(NFTQueLocal.length) - 1; i >= 0; i--) {
                 if (NFTQueLocal[uint(i)].user == _user) {
        removeFirst(uint(i));
    }
        }
    }

    function removeFirst(uint _id) internal {
        require(_id < NFTQue.length, "REMOVE_QUEUE: invalid index");

        for (uint i = _id; i < NFTQue.length - 1; i++) {
            NFTQue[i] = NFTQue[i + 1];
        }
