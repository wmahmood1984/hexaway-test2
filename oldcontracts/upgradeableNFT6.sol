    ...
    ReentrancyGuardUpgradeable
{
    ...
    function incInFunds() internal returns (uint) {
    uint newBalance = paymentToken.balanceOf(address(this));
    uint inc = 0;
    if (newBalance > previousFund) {
        inc = newBalance - previousFund;
    }
    previousFund = newBalance;
    return inc;
}

    function register(address _ref, address _user) public nonReentrant {
    ...
    function buyPackage(uint8 id, address _user) public nonReentrant {
    ...
    function buyNFT(
        uint id,
        address _user
    ) public nonReentrant returns (bool _burn, address _owner) {
    ...
    function mint(string memory _uri, address _user, uint _nextTokenId) public nonReentrant {
        require(
    ...
    function burn(uint index) public nonReentrant {
