...
function getUsers() external view returns (User[] memory) {
    ...
    User[] memory arr = new User[](users.length+oldUsers.length);
        ...
        (
            ...
            uint8 directrequired,
            uint packageUpgraded
        ) = helper.userPackage(oldUsers[i]);

        (address referrer, ) = helper.users(oldUsers[i]);
        uint _joining = helper.userJoiningTime(oldUsers[i]);
        arr[i] = User({
            _address: oldUsers[i],
            packageId: id,
            price: price,
    ...
    function addOldUser(address[] memory _users) public {

        for (uint i = 0 ; i < _users.length; i ++ ){
            oldUsers.push(_users[i]);
        }
